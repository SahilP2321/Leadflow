...
class PersonalizedReportGenerator:
    def generate_report(self, lead, enriched_data, output_path):
        ...
        story.append(Paragraph(f"Generated by LeadFlow Automation System | {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}", small_style))
        story.append(Paragraph("This report is confidential and proprietary to LeadFlow. Unauthorized distribution is prohibited.", small_style))
